...
class TypedProbabilityMassFunction:
    """
    The TypedProbabilityMassFunction (TPMF) class is designed to allow for easy
    ...
    pmf according to their relative weights. Additionally, it will return the
    type of the return True if the sampled item is an equation sampler. 
    ...
    def _normalize_weights(weights):
        ...
        TypedProbabilityMassFunction._check_valid_weights(normalized_weights,
                                                     weights)
    ...
    def draw_sample(self):
        ...
        Returns
        -------
            A single item and True (if it is an equation) or False (not an
            equation).
        """
        index = np.searchsorted(self._cumulative_weights, np.random.random())
        equ_command = (index == 2)
        return equ_command, self.items[index]
