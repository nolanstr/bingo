...
class LexicaseIsland(EvolutionaryOptimizer):
    """
    ...
    def __init__(self, evolution_algorithm, generator, population_size,
                 update_frequency=1, subset_size=20,
    ...
    def _update_case(self):
        pos_idxs = np.arange(self._full_td.x.shape[0])
        case_idxs = np.random.choice(pos_idxs, size=self._subset_size, 
                                                            replace=False)
