...
class ContinuousLocalOptimization(FitnessFunction):
    ...
    def _loss(self, individual, x0):
        
        individual.set_local_optimization_params(x0)
        y_approx = individual.evaluate_equation_at(self.training_data.x)
        fits = []
        for i in range(self.idxs.shape[0]-1):
            fits.append(np.mean(abs(self.training_data.y[self.idxs[i]:self.idxs[i+1]]
                - y_approx[self.idxs[i]:self.idxs[i+1]])))
        return np.sum(fits)
    ...
    def _run_algorithm_for_optimization(self, sub_routine, individual, params):
        ...
        if self._algorithm in ROOT_SET:
            optimize_result = optimize.minimize(
                    lambda x: self._loss(individual, x),
                    params) 
