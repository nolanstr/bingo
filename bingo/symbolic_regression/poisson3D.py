...
def analytic_solution(X, k):
    return np.sin(k * X[:, 0]) * np.sin(k * X[:, 1])* np.sin(k * X[:, 2])
    ...
    def pdefn(X, U):
        ...
        if U.grad_fn is not None:
            ...
            u_z = torch.autograd.grad(
                U.sum(), X[2], create_graph=True, allow_unused=True)[0]

            if u_x is not None or u_y is not None or u_z is not None:
                pen_mult = np.inf

            if u_x is not None and u_y is not None and u_z is not None:
               pen_mult = np.inf
               ...
               if u_z.grad_fn is not None:
                  u_zz = torch.autograd.grad(
                       u_z.sum(), X[2],  create_graph=True, allow_unused=True)[0]
               else:
                  u_zz = torch.zeros_like(U)

               if u_xx is not None and u_yy is not None and u_zz is not None:

                  return [u_xx + u_yy + u_zz + 3. * (k**2.) * torch.sin(k*X[0]) * torch.sin(k*X[1])* torch.sin(k*X[2])]

        return [torch.ones_like(U) * pen_mult]

    return pdefn

def gen_training_data(k, low=[0, 0, 0], high=[1, 1, 1], n_b=16, n_df=64):

    X_boundary = util.boundary_X_3d(low, high, n_b)
    U_boundary = analytic_solution(X_boundary, k)[:, None]

    X_df = np.random.uniform(low=low, high=high, size=(n_df, 3))
