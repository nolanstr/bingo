...
from ...util.typed_probability_mass_function import TypedProbabilityMassFunction
...
class ComponentGenerator:
    ...
    def __init__(
        ...
        self._operator_pmf = TypedProbabilityMassFunction()
        self._equation_pmf = TypedProbabilityMassFunction()
    ...
    def _make_terminal_pdf(self, constant_probability):
        ...
        return TypedProbabilityMassFunction(items=[1, 0], weights=terminal_weight)
    ...
    def _make_random_command_pmf(
        ...
        return TypedProbabilityMassFunction(
            items=[
    ...
    def random_command(self, stack_location, valid_parameters):
    ...
    def random_command_w_eq(self, stack_location, valid_parameters=None):
        ...
        """
        if stack_location < self._num_initial_load_statements:
            return False, self.random_terminal_command(stack_location, valid_parameters)
        equ_command, command_pdf = self._full_random_command_function_pmf.draw_sample()
        command = command_pdf(stack_location, valid_parameters)
        if len(command.shape) == 1:
            command = np.array([command])
        return equ_command, command

    def random_operator_command(self, stack_location, valid_parameters):
        ...
        return np.array(
            [
                self.random_operator()[1],
                self.random_operator_parameter(valid_parameters),
                self.random_operator_parameter(valid_parameters),
            ],
            dtype=int,
        )
    ...
    @staticmethod
    def random_operator_parameter(valid_parameters):
        """Get random operator parameter

        Parameters
        ----------
        valid_parameters : int
            locations of valid parameters in stack
        ...
        stack_location.
        """
        return np.random.choice(valid_parameters)

    def random_terminal_command(self, _=None, valid_parameters=None):
        ...
        """
        _, terminal = self.random_terminal()
    ...
    def random_equation(self, stack_location, valid_parameters):
        _, command_array = self._equation_pmf.draw_sample()
