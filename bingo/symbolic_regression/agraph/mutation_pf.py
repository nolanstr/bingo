...
class AGraphMutation(Mutation):
    ...
    def __init__(self, pf_fitness, component_generator, command_probability=0.2,
                 node_probability=0.2, parameter_probability=0.2,
                 prune_probability=0.2, fork_probability=0.2, n=4):
    ...
    def __call__(self, parent):
        ...
        for i in range(self.n):
            ...
            pf_fit = self.pf_fitness(child)
            pf_fitness.append(pf_fit)

        pf_fitness = np.array(pf_fitness) 
        if np.all(pf_fitness==0):
            weights = np.ones(pf_fitness.shape) / pf_fitness.shape[0]
        else:
            weights = pf_fitness / pf_fitness.sum()
        selected_child = np.random.choice(children, p=weights)
