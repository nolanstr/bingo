...
"""
I think we may need to completely rewrite this for full seeded approach...
"""
class AGraphMutation(Mutation):
            ...
            if not IS_TERMINAL_MAP[node]:
                if p_1 == mutation_location:
                    individual.mutable_command_array[
                        mutation_location + i, 1
                    ] = pruned_param
                if p_2 == mutation_location:
                    individual.mutable_command_array[
                        mutation_location + i, 2
                    ] = pruned_param
    ...
    @staticmethod
    def _move_utilized_commands(
        stack, utilized_commands, mutation_location, comparator=lambda i, loc: i <= loc
    ):
        """
    ...
    def _fix_indices(self, stack, utilized_commands, index_shifts):
        ...
        for i in range(1, 3):
            ...
            if 0 in indices_to_fix:  # case where we have a non-terminal at i=0
                ...
                indices_to_fix = np.delete(
                    indices_to_fix, np.where(indices_to_fix == 0)
                )
    ...
    def _eq_mutation(self, individual):
        ...
        if len(eq) > utilized_commands.count(False) or mutation_location is None:
            return
        else:
            stack = individual.mutable_command_array
