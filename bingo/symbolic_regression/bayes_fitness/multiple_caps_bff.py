...
class BayesFitnessFunction(FitnessFunction, Utilities, Priors, RandomSample,
        Statistics):
    def __init__(self, continuous_local_opt, caps, smc_hyperparams={},
    ...
    def __call__(self, individual, return_nmll_only=True):
        ...
        priors = [priors[i] for i in range(n_params)] + \
            [ImproperUniform(0, cap) for cap in self._caps] 
        try:
        ...
        if self._model_check(individual):
            ...
            for i in range(self._ensemble):
                nmll, step_list = self._estimate_nmll(individual, smc, proposal)
                nmlls[i] = nmll
            
            if return_nmll_only:
                return np.nanmean(nmlls)
            else:
                return np.nanmean(nmlls), step_list, vector_mcmc 
    ...
    def do_local_opt(self, individual, subset):
        if not self.bingocpp:
            ...
            ind = individual.copy()
            if subset is None:
                _ = self._cont_local_opt(ind)
                individual.set_local_optimization_params(
                                ind.get_local_optimization_params())
