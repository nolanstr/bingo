...
class BayesFitnessFunction(FitnessFunction, Utilities, Priors, RandomSample,
    ...
    def check_physics(self, ind):
        ...
        if np.any(f_init<0): #checks for negative initial conditions
            return False
        f, df = ind.evaluate_equation_with_x_gradient_at(self.training_data.x)
        #if np.any(df[:,0]<0):
        #    return False
        
        d2f = [] 
        for idxs in self.full_idxs:
            X = self.training_data.x[idxs,:]
            X = X[X[:, 0].argsort()]
            f, df = ind.evaluate_equation_with_x_gradient_at(X)
            d2f.append(np.diff(df[:,0])/np.diff(X[:,0]))
