...
class ImplicitLikelihood(BaseLogLike):
    ...
    def estimate_likelihood(self, inputs):
        ...
        for i, val in enumerate(vals):
            ...
            f, df_dx = val
            l1_norm = np.linalg.norm(df_dx,axis=1,ord=1).reshape((-1,1))
            l2_norm = np.linalg.norm(df_dx,axis=1,ord=2).reshape((-1,1))
            num = np.square(f) 
            den = np.square(l2_norm)
            ssqe_base = np.sum(np.square(
                    np.linalg.norm(df_dx*f/den, axis=1, ord=2)))
            #reg_base = np.linalg.norm(df_dx * np.mean(l1_norm/l2_norm,
            #    axis=1).reshape((-1,1)),axis=1)
            
            #reg = np.exp(-np.mean(reg_base/reg_base.mean())) / np.exp(-1)
            zero_idxs = np.where(df_dx==0)
            reg2 = np.zeros_like(df_dx)
            reg2[zero_idxs] += abs(f[zero_idxs[0]].flatten())
            ssqe[i] = ssqe_base + reg2.sum()
